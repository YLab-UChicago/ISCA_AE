...
def set_replay_pos(target, rp, train_recorder, db_st):
    shape = target.shape
    ...
    for i in range(len(positions)):
        ...
        train_recorder.write("Position is {}, Golden data is {}, inject data is {}\n".format(pos, ori_val, val_delta))
        train_recorder.flush()
        db_st.inj_values.append(val_delta)
...
def get_replay_args(inj_type, rp, strategy, inj_layer, inputs, kernels, outputs, train_recorder, db_st):
    np.random.seed(None)
    #inj_replica = np.random.randint(strategy.num_replicas_in_sync)
    inj_replica = rp.target_worker
    db_st.target_worker = rp.target_worker
    record(train_recorder, "Inject worker: {}\n".format(inj_replica))
    record(train_recorder, "Inject layer: {}\n".format(inj_layer))
    ...
    mask, delta = set_replay_pos(target, rp, train_recorder, db_st)
